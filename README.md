...
시각장애인의 청각을 가리지 않으면서 보행 경로의 좌·우 회전과 주변 장애물을 진동으로 전달하기 위한 시제품입니다. 기존 `iot_example`의 카카오 장소 검색·도보 경로 호출을 유지하면서, 장소 선택, 지도 중심 레이아웃(데스크톱 플로팅 패널 / 모바일 하단 시트), GPS 기반 회전·현재 위치 실시간 추적, 진동 명령, ToF 텔레메트리를 하나의 흐름으로 연결했습니다.
...
9. **경로 기록 / 기록 경로 안내 API** *(백엔드에만 구현, 현재 웹 화면 UI에는 연결되어 있지 않음)*: 보호자가 실제 경로를 걸으며 좌회전·우회전·횡단보도·계단·도착 지점을 태깅해 `recorded_routes/route_N.json`으로 저장하고(`/api/recordings/*`), 저장된 경로로 다시 안내받는(`/api/recorded-routes/*`) 기능이 `recording.py`/`recorded_navigation.py`에 구현돼 있습니다. 웹 UI 개편 과정에서 화면에서는 빠졌고 API로만 호출 가능합니다.
10. **위험 버튼 알림**: 시각장애인 쪽 위험 버튼(시뮬레이터)을 누르면 보호자 화면에 빨간 배너로 알림 표시 (페이지가 열려 있는 동안 4초마다 폴링)
11. **지도 중심 레이아웃**: 지도가 화면 전체를 채우고 검색·경로 패널이 그 위에 얹힙니다 — 데스크톱은 좌측에 살짝 반투명하게 뜬 카드, 모바일은 손잡이를 잡고 위아래로 끌어 크기를 조절할 수 있는 하단 시트
12. **경로 구간 강조**: 안내 목록의 번호를 누르면 지도에서 그 구간만 강조 표시(시작·끝 지점 번호 마커 포함). 계단·횡단보도 구간은 경로가 만들어지는 시점부터 항상 주황/빨강으로 상시 표시됩니다
13. **"전체 경로" / "현재 이동 경로" 보기 전환**: GPS 안내 중에만 뜨는 지도 위 토글. "현재 이동 경로"를 선택하면 실제 GPS 진행에 맞춰 강조 구간이 자동으로 다음 단계로 넘어가고, 지도 화면도 신호가 올 때마다 현재 위치를 따라 움직입니다("전체 경로"에서는 화면이 고정되고 목록을 수동으로 눌러 구간을 미리볼 수 있습니다)
14. **ETA 바**: GPS 안내 중 지도 하단 중앙에 남은 시간·거리·도착 예정 시각이 실시간으로 표시됩니다
15. **현재 위치 버튼**: 지도 우하단 버튼으로 언제든 GPS 현재 위치로 지도를 재중심할 수 있습니다
...
│  ├─ index.html
│  ├─ styles.css
│  └─ app.js                  검색·지도·GPS·센서 UI, 경로 구간 강조·자동 추적
├─ docs/
...
브라우저에서 `http://localhost:8000`을 엽니다. 지도가 화면 전체를 채우고, 검색·경로 패널은 그 위에 얹혀 있습니다(데스크톱은 좌측 카드, 모바일은 하단 시트 — 손잡이를 끌어 크기 조절 가능).

- **경로** 탭: 출발지·도착지 검색 → 후보 선택(또는 "현위치") → 경로 생성 → 안내 목록에서 번호를 눌러 구간 미리보기 → GPS 안내 시작. 안내 중에는 지도 위 토글로 "전체 경로"(화면 고정) / "현재 이동 경로"(자동 추적) 전환.
- **센서** 탭: ToF 4개 값을 직접 넣어 전송해보는 시뮬레이터, 위험 버튼 시뮬레이터.
...
- 환경변수: `KAKAO_REST_API_KEY`, `KAKAO_JAVASCRIPT_KEY` (Render 대시보드의 Environment에 등록)
- 카카오 개발자 콘솔의 JavaScript 키 허용 도메인에 **배포된 Render 주소**를 등록해야 지도가 뜹니다. 등록 전에는 "카카오 지도 SDK를 불러오지 못했습니다" 메시지만 표시됩니다.
- 서버 상태는 메모리에 있으므로(경로·진동 버퍼 등) Render가 인스턴스를 재시작하면 초기화됩니다. `recorded_routes/route_N.json`처럼 파일로 저장된 것만 남습니다.
- 정적 파일(`web/`)의 캐시를 확실히 갱신하려면 `index.html`의 `styles.css?v=...` / `app.js?v=...` 쿼리 문자열을 수정할 때마다 값을 바꾸세요 — 값이 그대로면 브라우저가 이전 파일을 계속 캐시해서 배포해도 반영이 안 되는 것처럼 보일 수 있습니다.
