...
from .recorded_navigation import RecordedNavigationService, RecordedRouteNotStartedError
...
# ---------------------------------------------------------------------------
# 경로 안내 모드 (Guidance over a recorded route)
#
# Loads a route_N.json produced by 경로 기록 모드 and, as the walker's GPS
# updates come in, calculates distance to the next tagged waypoint and
# publishes haptic commands through the same CommandBuffer /api/haptics
# already polls. This still uses the browser's GPS as a stand-in for the
# belt's GPS module.
# ---------------------------------------------------------------------------


@app.post("/api/recorded-routes/{route_id}/start")
def start_recorded_navigation(request: Request, route_id: int) -> dict[str, object]:
    try:
        session = request.app.state.recorded_navigation.start(route_id)
    except RecordingNotFoundError as exc:
        raise HTTPException(
            status_code=404, detail="저장된 경로를 찾을 수 없습니다."
        ) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return session.to_public_dict()
...
@app.post("/api/recorded-routes/{route_id}/location")
def update_recorded_navigation_location(
    request: Request,
    route_id: int,
    body: LocationBody,
) -> dict[str, object]:
    try:
        result = request.app.state.recorded_navigation.update(
            route_id,
            Coordinate(body.longitude, body.latitude),
            body.accuracy_meters,
        )
    except RecordedRouteNotStartedError as exc:
        raise HTTPException(
            status_code=409, detail="먼저 안내를 시작하세요."
        ) from exc
    command_objects = cast(list[HapticCommand], result.pop("_commandObjects"))
    _service(request).commands.publish(command_objects)
    return result
