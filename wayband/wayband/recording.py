...
class RecordingStore:
    ...
    def load_route(self, route_id: int) -> dict[str, Any]:
        """Read back a finished recording for guidance mode."""

        path = self._storage_dir / f"route_{route_id}.json"
        if not path.is_file():
            raise RecordingNotFoundError(route_id)
        try:
            return json.loads(path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            raise ValueError(f"route_{route_id}.json 형식이 올바르지 않습니다.") from exc
